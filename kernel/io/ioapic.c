...
static void ioapic_write(uint32_t reg, uint32_t val) { ioapic_base[IOAPIC_REGSEL / 4] = reg; ioapic_base[IOAPIC_WIN / 4] = val; }

uint32_t ioapic_max_redirects(void) { return ((ioapic_read(IOAPIC_VER) >> 16) & 0xFF) + 1; }
