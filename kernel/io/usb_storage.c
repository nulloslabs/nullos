...
static uint32_t storage_next_tag = 1;
usb_storage_entry_t storage_entries[USB_STORAGE_MAX_DEVICES];
...
static uint32_t get_next_tag(void) {
    uint32_t tag = __sync_fetch_and_add(&storage_next_tag, 1);
    while (tag == 0) tag = __sync_fetch_and_add(&storage_next_tag, 1);
    if (storage_next_tag == 0) storage_next_tag = 1;
    return tag;
}

static int find_storage_entry(usb_hcd_t *hcd, uint8_t port_id) {
    for (int i = 0; i < USB_STORAGE_MAX_DEVICES; i++) { if (storage_entries[i].present && storage_entries[i].hcd == hcd && storage_entries[i].dev && storage_entries[i].dev->port_id == port_id) return i; }
    return -1;
}

static int alloc_storage_entry(void) {
    for (int i = 0; i < USB_STORAGE_MAX_DEVICES; i++) { if (!storage_entries[i].present) return i; }
    return -1;
}
...
static int parse_storage_config(uint8_t *buf, uint16_t total_len, uint8_t speed, uint8_t *iface, uint8_t *in_ep, uint8_t *out_ep, uint16_t *in_mps, uint16_t *out_mps) {
...
static int transfer_bulk_chunks(usb_hcd_t *hcd, usb_device_t *dev, uint8_t ep, uint8_t *data, uint32_t len, uint32_t *done_out) {
    ...
    while (done < len) {
        ...
        if (chunk > USB_STORAGE_BULK_CHUNK) chunk = USB_STORAGE_BULK_CHUNK;
...
static int get_max_lun(usb_hcd_t *hcd, usb_device_t *dev, uint8_t iface) {
    ...
    setup.bRequest = USB_STORAGE_GET_MAX_LUN;
    setup.wValue = 0;
    setup.wIndex = iface;
    setup.wLength = 1;
    int ret = hcd->control_transfer(hcd, dev, &setup, &lun, 1);
    if (ret != 1) return 0;
    if (lun > USB_STORAGE_MAX_LUN) return 0;
    return (int)lun;
}
...
static int reset_storage_device(usb_hcd_t *hcd, usb_device_t *dev, uint8_t iface, uint8_t in_ep, uint8_t out_ep) {
    if (!hcd || !dev) return -1;
    usb_setup_packet_t setup;
    setup.bmRequestType = USB_REQTYPE_DIR_OUT | USB_REQTYPE_CLASS | USB_REQTYPE_INTERFACE;
    setup.bRequest = USB_STORAGE_MASS_RESET;
    setup.wValue = 0;
    setup.wIndex = iface;
    setup.wLength = 0;
...
static int storage_transfer(usb_hcd_t *hcd, usb_device_t *dev, uint8_t iface, uint8_t lun, uint8_t *cdb, uint8_t cdb_len, uint8_t *data, uint32_t data_len, bool dir_in, uint8_t *csw_status) {
    if (!hcd || !dev || !cdb || cdb_len == 0 || cdb_len > 16) return -1;
    if (data_len && !data) return -1;
    if (lun > USB_STORAGE_MAX_LUN) return -1;
    uint8_t in_ep = dev->bulk_in_endpoint;
    uint8_t out_ep = dev->bulk_out_endpoint;
    if ((in_ep & 0x80) == 0 || (out_ep & 0x80) != 0) return -1;
    usb_storage_cbw_t cbw;
    memset(&cbw, 0, sizeof(cbw));
    cbw.signature = USB_STORAGE_CBW_SIGNATURE;
    ...
    int ret = hcd->bulk_transfer(hcd, dev, out_ep, &cbw, USB_STORAGE_CBW_LENGTH);
    if (ret != USB_STORAGE_CBW_LENGTH) { reset_storage_device(hcd, dev, iface, in_ep, out_ep); return -1; }
    uint32_t done = 0;
    if (data_len) {
        if (dir_in) {
            ...
            if (dret < 0 || done != data_len) { clear_bulk_halt(hcd, dev, out_ep); reset_storage_device(hcd, dev, iface, in_ep, out_ep); return -1; }
        }
    }
    usb_storage_csw_t csw;
    for (int i = 0; i < USB_STORAGE_CSW_RETRIES; i++) {
        memset(&csw, 0, sizeof(csw));
        ret = hcd->bulk_transfer(hcd, dev, in_ep, &csw, USB_STORAGE_CSW_LENGTH);
        if (ret != USB_STORAGE_CSW_LENGTH) { clear_bulk_halt(hcd, dev, in_ep); continue; }
        if (csw.signature != USB_STORAGE_CSW_SIGNATURE || csw.tag != cbw.tag) { reset_storage_device(hcd, dev, iface, in_ep, out_ep); return -1; }
        if (csw.residue > data_len) { reset_storage_device(hcd, dev, iface, in_ep, out_ep); return -1; }
        if (csw.status == USB_STORAGE_STATUS_PASS) {
            if (csw.residue != data_len - done) { log("usb storage: residue mismatch (%u != %u) on port %d\n", csw.residue, data_len - done, dev->port_id); reset_storage_device(hcd, dev, iface, in_ep, out_ep); return -1; }
            if (done != data_len) { log("usb storage: short data phase (%u of %u bytes) on port %d\n", done, data_len, dev->port_id); return -1; }
            return 0;
        }
        if (csw.status == USB_STORAGE_STATUS_FAIL) { if (csw_status) *csw_status = csw.status; return -1; }
        if (csw.status == USB_STORAGE_STATUS_PHASE) { reset_storage_device(hcd, dev, iface, in_ep, out_ep); return -1; }
        return -1;
    }
    reset_storage_device(hcd, dev, iface, in_ep, out_ep);
    return -1;
}
...
static int request_sense_data(usb_hcd_t *hcd, usb_device_t *dev, uint8_t iface, uint8_t lun, uint8_t *buf) {
    ...
    cdb[4] = USB_STORAGE_SENSE_LENGTH;
    return storage_transfer(hcd, dev, iface, lun, cdb, 6, buf, USB_STORAGE_SENSE_LENGTH, true, NULL);
}

static void report_storage_sense(usb_hcd_t *hcd, usb_device_t *dev, uint8_t iface, uint8_t lun) {
    uint8_t sense[USB_STORAGE_SENSE_LENGTH];
    memset(sense, 0, sizeof(sense));
    if (request_sense_data(hcd, dev, iface, lun, sense) != 0) return;
    log("usb storage: command failed (port %d, lun %d)\n", dev->port_id, lun);
}

static int run_storage_command(usb_hcd_t *hcd, usb_device_t *dev, uint8_t iface, uint8_t lun, uint8_t *cdb, uint8_t cdb_len, uint8_t *data, uint32_t data_len, bool dir_in) {
    uint8_t csw_status = 0;
    int ret = storage_transfer(hcd, dev, iface, lun, cdb, cdb_len, data, data_len, dir_in, &csw_status);
    if (ret < 0 && csw_status == USB_STORAGE_STATUS_FAIL) report_storage_sense(hcd, dev, iface, lun);
    return ret;
}
...
static int test_unit_ready(usb_hcd_t *hcd, usb_device_t *dev, uint8_t iface, uint8_t lun) {
    ...
    return run_storage_command(hcd, dev, iface, lun, cdb, 6, NULL, 0, false);
}
...
static int get_inquiry_data(usb_hcd_t *hcd, usb_device_t *dev, uint8_t iface, uint8_t lun, uint8_t *buf) {
    ...
    cdb[4] = USB_STORAGE_INQUIRY_LENGTH;
    return run_storage_command(hcd, dev, iface, lun, cdb, 6, buf, USB_STORAGE_INQUIRY_LENGTH, true);
}
...
static int get_device_capacity(usb_hcd_t *hcd, usb_device_t *dev, uint8_t iface, uint8_t lun, uint64_t *count, uint32_t *size) {
    ...
    uint8_t cdb[10];
    uint8_t buf[USB_STORAGE_CAPACITY_LENGTH];
    memset(cdb, 0, sizeof(cdb));
    memset(buf, 0, sizeof(buf));
    cdb[0] = USB_SCSI_READ_CAPACITY10;
    int ret = run_storage_command(hcd, dev, iface, lun, cdb, 10, buf, USB_STORAGE_CAPACITY_LENGTH, true);
    if (ret != 0) return -1;
    ...
    if (last == 0xFFFFFFFF) {
        ...
        ret = run_storage_command(hcd, dev, iface, lun, cdb16, 16, buf16, 32, true);
        if (ret != 0) return -1;
...
static int read_storage_blocks(usb_storage_entry_t *entry, uint64_t lba, uint16_t blocks, uint8_t *buf) {
    ...
    if (lba <= 0xFFFFFFFF) {
        ...
        return run_storage_command(entry->hcd, entry->dev, entry->interface_number, entry->lun, cdb, 10, buf, total, true);
    }
    uint8_t cdb16[16];
    memset(cdb16, 0, sizeof(cdb16));
    cdb16[0] = 0x88;
    ...
    return run_storage_command(entry->hcd, entry->dev, entry->interface_number, entry->lun, cdb16, 16, buf, total, true);
}

static int write_storage_blocks(usb_storage_entry_t *entry, uint64_t lba, uint16_t blocks, uint8_t *buf) {
    if (!entry || !entry->dev || !entry->hcd || !buf || blocks == 0) return -1;
    uint32_t blen = entry->block_size;
    if (blen != 512 && blen != 1024 && blen != 2048 && blen != 4096) return -1;
    uint32_t total = (uint32_t)blocks * blen;
    if (lba <= 0xFFFFFFFF) {
        ...
        write_be32(cdb + 2, (uint32_t)lba);
        write_be16(cdb + 7, blocks);
        return run_storage_command(entry->hcd, entry->dev, entry->interface_number, entry->lun, cdb, 10, buf, total, false);
    }
    uint8_t cdb16[16];
    memset(cdb16, 0, sizeof(cdb16));
    cdb16[0] = 0x8A;
    ...
    cdb16[12] = (uint8_t)(blocks >> 8);
    cdb16[13] = (uint8_t)blocks;
    return run_storage_command(entry->hcd, entry->dev, entry->interface_number, entry->lun, cdb16, 16, buf, total, false);
}

static int probe_storage_lun(usb_hcd_t *hcd, usb_device_t *dev, uint8_t iface, uint8_t lun) {
    bool ready = false;
    for (int i = 0; i < USB_STORAGE_TUR_RETRIES; i++) {
        if (test_unit_ready(hcd, dev, iface, lun) == 0) { ready = true; break; }
        report_storage_sense(hcd, dev, iface, lun);
        sleep(100);
    }
    if (!ready) { log("usb storage: lun %d never became ready (port %d)\n", lun, dev->port_id); return -1; }
    uint8_t inquiry[USB_STORAGE_INQUIRY_LENGTH];
    memset(inquiry, 0, sizeof(inquiry));
    get_inquiry_data(hcd, dev, iface, lun, inquiry);
    uint64_t blocks = 0;
    uint32_t blen = 0;
    if (get_device_capacity(hcd, dev, iface, lun, &blocks, &blen) != 0) { log("usb storage: read capacity failed (lun %d, port %d)\n", lun, dev->port_id); return -1; }
    if (blocks == 0 || blen == 0 || blocks > (uint64_t)-1 / blen) { log("usb storage: invalid capacity (lun %d, port %d)\n", lun, dev->port_id); return -1; }
    uint64_t total = blocks * blen;
    int slot = alloc_storage_entry();
    if (slot < 0) { log("usb storage: no free device slots, dropping disk (lun %d, port %d)\n", lun, dev->port_id); return -1; }
    char disk_name[16];
    bool named = false;
    for (int i = 0; i < 26; i++) {
        char cand[16];
        if (!make_usb_storage_name(cand, sizeof(cand), i)) continue;
        uint64_t dummy = 0;
    ...
    if (!named) { log("usb storage: no free disk names, dropping disk (lun %d, port %d)\n", lun, dev->port_id); return -1; }
    storage_entries[slot].dev = dev;
    storage_entries[slot].hcd = hcd;
    storage_entries[slot].interface_number = iface;
    storage_entries[slot].lun = lun;
    storage_entries[slot].block_size = blen;
    storage_entries[slot].block_count = blocks;
    storage_entries[slot].total_size = total;
    storage_entries[slot].present = true;
    memcpy(storage_entries[slot].name, disk_name, sizeof(disk_name));
    if (register_disk_device_idx(disk_name, read_usb_storage_device, write_usb_storage_device, slot, total, DISK_BUS_USB) < 0) {
        log("usb storage: failed to register disk (lun %d, port %d)\n", lun, dev->port_id);
        memset(&storage_entries[slot], 0, sizeof(storage_entries[slot]));
        return -1;
    }
...
int get_usb_storage_count(void) {
    int count = 0;
    for (int i = 0; i < USB_STORAGE_MAX_DEVICES; i++) { if (storage_entries[i].present) count++; }
    return count;
}

uint64_t get_usb_storage_size(int index) {
    if (index < 0 || index >= USB_STORAGE_MAX_DEVICES) return 0;
    if (!storage_entries[index].present) return 0;
    return storage_entries[index].total_size;
}

uint32_t get_usb_storage_block_size(int index) {
    if (index < 0 || index >= USB_STORAGE_MAX_DEVICES) return 0;
    if (!storage_entries[index].present) return 0;
    if (storage_entries[index].block_size != 512 && storage_entries[index].block_size != 1024 && storage_entries[index].block_size != 2048 && storage_entries[index].block_size != 4096) return 0;
    return storage_entries[index].block_size;
}
...
uint64_t read_usb_storage_device(void *buf, uint64_t count, uint64_t offset, int index, void *handle) {
    (void)handle;
    if (index < 0 || index >= USB_STORAGE_MAX_DEVICES || !storage_entries[index].present) return (uint64_t)-ENODEV;
    if (!count) return 0;
    if (!buf) return (uint64_t)-EINVAL;
    usb_storage_entry_t *entry = &storage_entries[index];
    ...
    while (done < count) {
        ...
        if (off == 0 && left >= blen) {
            ...
            if (blocks > USB_STORAGE_MAX_BLOCKS_PER_CMD) blocks = USB_STORAGE_MAX_BLOCKS_PER_CMD;
            if (read_storage_blocks(entry, lba, (uint16_t)blocks, out + done) != 0) { free(tmp); return (uint64_t)-EIO; }
            done += blocks * blen;
            continue;
        }
        if (read_storage_blocks(entry, lba, 1, tmp) != 0) { free(tmp); return (uint64_t)-EIO; }
...
uint64_t write_usb_storage_device(const void *buf, uint64_t count, uint64_t offset, int index, void *handle) {
    ...
    while (done < count) {
        ...
        if (off == 0 && left >= blen) {
            uint64_t blocks = left / blen;
            if (blocks > USB_STORAGE_MAX_BLOCKS_PER_CMD) blocks = USB_STORAGE_MAX_BLOCKS_PER_CMD;
            if (write_storage_blocks(entry, lba, (uint16_t)blocks, (uint8_t *)in + done) != 0) { free(tmp); return (uint64_t)-EIO; }
            done += blocks * blen;
            continue;
        }
        if (read_storage_blocks(entry, lba, 1, tmp) != 0) { free(tmp); return (uint64_t)-EIO; }
        ...
        if (write_storage_blocks(entry, lba, 1, tmp) != 0) { free(tmp); return (uint64_t)-EIO; }
        done += chunk;
    }
    free(tmp);
    return count;
...
void remove_usb_storage(usb_hcd_t *hcd, uint8_t port_id) {
    ...
    for (int i = 0; i < USB_STORAGE_MAX_DEVICES; i++) {
        if (!storage_entries[i].present || storage_entries[i].hcd != hcd || !storage_entries[i].dev || storage_entries[i].dev->port_id != port_id) continue;
        remove_gpt_partitions(i, DISK_BUS_USB);
        remove_mbr_partitions(i, DISK_BUS_USB);
        unregister_device(storage_entries[i].name);
        dev = storage_entries[i].dev;
        memset(&storage_entries[i], 0, sizeof(storage_entries[i]));
    }
...
void init_usb_storage(usb_hcd_t *hcd, uint8_t speed, uint8_t port_id) {
    ...
    if (find_storage_entry(hcd, port_id) >= 0) return;
    usb_device_t *dev = usb_allocate_device();
    if (!dev) {
        log("usb storage: failed to allocate device slot\n");
        return;
    }
    ...
    if (!desc_buf || !cfg_buf) {
        log("usb storage: failed to allocate descriptor buffers (port %d)\n", port_id);
        goto init_fail_dev;
    }
    ...
    if (new_address < 0) { log("usb storage: no free device addresses left (port %d)\n", port_id); goto init_fail_dev; }
    dev->address = 0;
    dev->speed = speed;
    ...
    if (retry_control_transfer(hcd, dev, &setup, desc_buf, 18, 4, 10) < 0) {
        log("usb storage: failed to read initial descriptor (port %d)\n", port_id);
    ...
    if (retry_control_transfer(hcd, dev, &setup, NULL, 0, 3, 10) < 0) {
        log("usb storage: set_address failed (port %d)\n", port_id);
        goto init_fail_address;
    }
    ...
    if (retry_control_transfer(hcd, dev, &setup, desc_buf, 18, 5, 20) < 0) {
        log("usb storage: failed to read device descriptor (port %d)\n", port_id);
        goto init_fail_dev;
    }
    ...
    if (cfg_len > 9) {
        ...
        if (retry_control_transfer(hcd, dev, &setup, cfg_buf, cfg_len, 4, 10) < 0) {
            log("usb storage: failed to read full config descriptor (port %d)\n", port_id);
            goto init_fail_dev;
        }
    }
    ...
    uint16_t in_mps = 0;
    uint16_t out_mps = 0;
    if (parse_storage_config(cfg_buf, total_len, dev->speed, &iface, &in_ep, &out_ep, &in_mps, &out_mps) < 0) {
        goto init_fail_dev;
    }
    ...
    if (hcd->control_transfer(hcd, dev, &setup, NULL, 0) < 0) {
        log("usb storage: set_configuration failed (port %d)\n", port_id);
        goto init_fail_dev;
    }
    sleep(100);
    ...
    int found = 0;
    for (int lun = 0; lun <= max_lun && lun <= USB_STORAGE_MAX_LUN; lun++) {
        if (probe_storage_lun(hcd, dev, iface, (uint8_t)lun) >= 0) found++;
        else log("usb storage: lun %d not usable (port %d)\n", lun, port_id);
    }
    if (found == 0) {
        log("usb storage: no usable luns (port %d)\n", port_id);
        goto init_fail_dev;
    }
    free(desc_buf);
    free(cfg_buf);
