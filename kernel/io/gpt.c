...
static bool make_nvme_gpt_partition_name(char *name, uint64_t name_size, const char *disk_name, int number) {
    // nvme0n1 -> nvme0n1p1
    uint64_t length = strlen(disk_name);
    char digits[12];
    uint64_t digit_count = 0;
...
static bool read_gpt_header(int disk_index, uint64_t header_lba, uint64_t disk_sectors, gpt_header_t *header, disk_device_bus_t bus) {
    bool is_pata = bus == DISK_BUS_PATA;
...
static uint8_t *read_gpt_entries(int disk_index, uint64_t disk_sectors, const gpt_header_t *header, uint64_t *entries_size, disk_device_bus_t bus) {
    bool is_pata = bus == DISK_BUS_PATA;
    uint64_t sector_size = is_pata ? PATA_SECTOR_SIZE : SATA_SECTOR_SIZE;
...
static bool register_gpt_partition(int disk_index, const char *disk_name, int number, uint64_t first_lba, uint64_t last_lba, uint64_t disk_sectors, disk_device_bus_t bus) {
    ...
    if (bus == DISK_BUS_NVME) {
        if (!make_nvme_gpt_partition_name(name, sizeof(name), disk_name, number)) return false;
    } else {
...
bool probe_gpt_for_sata_disk(int disk_index, const char *disk_name, uint64_t disk_size) {
    ...
    if (!read_gpt_header(disk_index, 1, disk_sectors, &header, DISK_BUS_SATA)) {
        if (!read_gpt_header(disk_index, disk_sectors - 1, disk_sectors, &header, DISK_BUS_SATA)) return false;
        using_backup = true;
    }
    uint64_t entries_size;
    uint8_t *entries = read_gpt_entries(disk_index, disk_sectors, &header, &entries_size, DISK_BUS_SATA);
    if (!entries && !using_backup) {
        if (!read_gpt_header(disk_index, disk_sectors - 1, disk_sectors, &header, DISK_BUS_SATA)) return false;
        entries = read_gpt_entries(disk_index, disk_sectors, &header, &entries_size, DISK_BUS_SATA);
        using_backup = true;
    }
    ...
    for (uint32_t index = 0; index < header.entry_count; index++) {
        ...
        if (register_gpt_partition(disk_index, disk_name, index + 1, entry->first_lba, entry->last_lba, disk_sectors, DISK_BUS_SATA)) found = true;
    }
    free(entries);
    return found;
}
...
bool probe_gpt_for_pata_disk(int disk_index, const char *disk_name, uint64_t disk_size) {
    ...
    gpt_header_t header;
    bool using_backup = false;
    if (!read_gpt_header(disk_index, 1, disk_sectors, &header, DISK_BUS_PATA)) {
        if (!read_gpt_header(disk_index, disk_sectors - 1, disk_sectors, &header, DISK_BUS_PATA)) return false;
        using_backup = true;
    }
    uint64_t entries_size;
    uint8_t *entries = read_gpt_entries(disk_index, disk_sectors, &header, &entries_size, DISK_BUS_PATA);
    if (!entries && !using_backup) {
        if (!read_gpt_header(disk_index, disk_sectors - 1, disk_sectors, &header, DISK_BUS_PATA)) return false;
        entries = read_gpt_entries(disk_index, disk_sectors, &header, &entries_size, DISK_BUS_PATA);
        using_backup = true;
    }
    if (!entries) return false;
    bool found = false;
    for (uint32_t index = 0; index < header.entry_count; index++) {
        ...
        if (is_gpt_guid_empty(entry->type_guid)) continue;
        if (entry->first_lba < header.first_usable_lba || entry->last_lba > header.last_usable_lba) continue;
        if (register_gpt_partition(disk_index, disk_name, index + 1, entry->first_lba, entry->last_lba, disk_sectors, DISK_BUS_PATA)) found = true;
    }
    free(entries);
    return found;
}

static bool read_gpt_header_nvme(int disk_index, uint64_t header_lba, uint64_t disk_sectors, gpt_header_t *header) {
...
static uint8_t *read_gpt_entries_nvme(int disk_index, uint64_t disk_sectors, const gpt_header_t *header, uint64_t *entries_size) {
    uint64_t sector_size = NVME_BLOCK_SIZE;
    ...
    *entries_size = (uint64_t)header->entry_count * header->entry_size;
    if (*entries_size > GPT_MAX_ENTRY_BYTES) return 0;
    uint64_t entry_sectors = (*entries_size + sector_size - 1) / sector_size;
...
bool probe_gpt_for_nvme_disk(int disk_index, const char *disk_name, uint64_t disk_size) {
    if (!disk_name || disk_size < NVME_BLOCK_SIZE * 2) return false;
    uint64_t disk_sectors = disk_size / NVME_BLOCK_SIZE;
    gpt_header_t header;
    bool using_backup = false;
    if (!read_gpt_header_nvme(disk_index, 1, disk_sectors, &header)) {
        if (!read_gpt_header_nvme(disk_index, disk_sectors - 1, disk_sectors, &header)) return false;
        using_backup = true;
    }
    uint64_t entries_size;
    uint8_t *entries = read_gpt_entries_nvme(disk_index, disk_sectors, &header, &entries_size);
    if (!entries && !using_backup) {
        if (!read_gpt_header_nvme(disk_index, disk_sectors - 1, disk_sectors, &header)) return false;
        entries = read_gpt_entries_nvme(disk_index, disk_sectors, &header, &entries_size);
        using_backup = true;
    }
    if (!entries) return false;
    bool found = false;
    for (uint32_t index = 0; index < header.entry_count; index++) {
        ...
        if (is_gpt_guid_empty(entry->type_guid)) continue;
        if (entry->first_lba < header.first_usable_lba || entry->last_lba > header.last_usable_lba) continue;
        if (register_gpt_partition(disk_index, disk_name, index + 1, entry->first_lba, entry->last_lba, disk_sectors, DISK_BUS_NVME)) found = true;
    }
    free(entries);
    return found;
}
...
static uint8_t *read_gpt_entries_usb(int disk_index, uint64_t disk_sectors, const gpt_header_t *header, uint64_t *entries_size) {
    ...
    *entries_size = (uint64_t)header->entry_count * header->entry_size;
    if (*entries_size > GPT_MAX_ENTRY_BYTES) return 0;
    uint64_t entry_sectors = (*entries_size + blen - 1) / blen;
    if (header->entry_lba >= disk_sectors || entry_sectors > disk_sectors - header->entry_lba) return 0;
    uint8_t *entries = malloc(*entries_size);
    if (!entries) return 0;
...
bool probe_gpt_for_usb_disk(int disk_index, const char *disk_name, uint64_t disk_size) {
    ...
    gpt_header_t header;
    bool using_backup = false;
    if (!read_gpt_header_usb(disk_index, 1, disk_sectors, &header)) { if (!read_gpt_header_usb(disk_index, disk_sectors - 1, disk_sectors, &header)) return false; using_backup = true; }
    uint64_t entries_size = 0;
    uint8_t *entries = read_gpt_entries_usb(disk_index, disk_sectors, &header, &entries_size);
    if (!entries && !using_backup) { if (!read_gpt_header_usb(disk_index, disk_sectors - 1, disk_sectors, &header)) return false; entries = read_gpt_entries_usb(disk_index, disk_sectors, &header, &entries_size); using_backup = true; }
    if (!entries) return false;
    bool found = false;
    for (uint32_t index = 0; index < header.entry_count; index++) {
        gpt_entry_t *entry = (gpt_entry_t *)(entries + (uint64_t)index * header.entry_size);
        if (is_gpt_guid_empty(entry->type_guid)) continue;
        if (entry->first_lba < header.first_usable_lba || entry->last_lba > header.last_usable_lba) continue;
        if (register_gpt_partition(disk_index, disk_name, index + 1, entry->first_lba, entry->last_lba, disk_sectors, DISK_BUS_USB)) found = true;
    }
    free(entries);
    return found;
}
...
void remove_gpt_partitions(int disk_index, disk_device_bus_t bus) {
    for (int i = 0; i < gpt_partition_count; i++) {
        if (!gpt_partitions[i].active || gpt_partitions[i].disk_index != disk_index || gpt_partitions[i].bus != bus) continue;
