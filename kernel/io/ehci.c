...
static int perform_ehci_control_transfer(usb_hcd_t *hcd, usb_device_t *dev, usb_setup_packet_t *setup, void *data, uint16_t length) {
    ...
    if (!done) {
        log("ehci: control transfer failed\n");
        return -1;
    }
    return actual;
}
...
static bool check_kbd_claim(usb_hcd_t *hcd, uint8_t port) {
    for (int i = 0; i < kbd_total; i++) {
        if (kbd_list[i].hcd == hcd && kbd_list[i].dev && kbd_list[i].dev->port_id == port) return true;
    }
    return false;
}

static void arm_ehci_kbd(ehci_controller_t *ctrl) {
...
static void enumerate_ehci_port(ehci_controller_t *ctrl, uint8_t port) {
    ...
    if (check_kbd_claim(&ctrl->hcd, port)) return;
    if (!reset_ehci_port(ctrl, port)) return;
...
static void take_ehci_ownership(ehci_controller_t *ctrl) {
    ...
    if (value & EHCI_LEGSUP_BIOS) {
        ...
        *legsup = value | EHCI_LEGSUP_OS;
        ...
        if (*legsup & EHCI_LEGSUP_BIOS) {
            log("ehci: bios handoff failed\n");
...
static bool allocate_ehci_resources(ehci_controller_t *ctrl) {
    ...
    if (!dma_raw || !frame_raw) {
        if (dma_raw) pfree(dma_raw);
        if (frame_raw) pfree(frame_raw);
        return false;
    }
...
static bool start_ehci_controller(ehci_controller_t *ctrl) {
    ...
    if (command & EHCI_CMD_RUN) {
        ...
        if (!halted) {
            log("ehci: controller didn't halt\n");
            return false;
        }
    }
    ...
    if (!reset_done) {
       log("ehci: controller reset timed out\n");
       return false;
    }

    uint32_t *frame = (uint32_t *)ctrl->frame_list;
    for (int i = 0; i < EHCI_FRAME_LIST_ENTRIES; i++) {
        frame[i] = (uint32_t)ctrl->interrupt_qh_phys | EHCI_QH_TYPE_QH;
    }
    ...
    if (!running) {
        log("ehci: controller refused to start\n");
        return false;
    }
...
void poll_ehci_ports(void) {
    for (int i = 0; i < ehci_count; i++) {
        ...
        if (!ctrl->interrupt_busy) arm_ehci_kbd(ctrl);
        for (uint8_t port = 0; port < ctrl->num_ports; port++) {
            uint32_t status = read_ehci_portsc(ctrl, port);
            if (status == UINT32_MAX) continue;
            if (status & EHCI_PORT_OWNER) continue;
...
void init_ehci(pci_device_t *dev) {
    ...
    if (bar_low == UINT32_MAX || (bar_low & 1u)) {
        log("ehci: bar0 is not a memory bar\n");
        return;
    }

    uint32_t bar_type = (bar_low >> 1) & 3u;
    if (bar_type != 0 && bar_type != 2) {
        log("ehci: unsupported bar0 type\n");
        return;
    }
    ...
    if (!mmio_phys) {
        log("ehci: invalid bar0 address\n");
        return;
    }
    for (int i = 0; i < ehci_count; i++) {
        if (ehci_controllers[i].mmio_phys == mmio_phys) return;
    }
