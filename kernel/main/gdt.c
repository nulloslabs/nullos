...
static void set_gdt_entry(uint64_t *gdt, int num, uint32_t base, uint32_t limit, uint8_t access, uint8_t gran) {
    ...
    gdt[num] |= ((base & 0xFFFFULL)      << 16);
    gdt[num] |= ((base >> 16 & 0xFFULL)  << 32);
    gdt[num] |= ((uint64_t)access        << 40);
    gdt[num] |= ((limit >> 16 & 0x0FULL) << 48);
    gdt[num] |= ((uint64_t)(gran & 0xF0) << 48);
    gdt[num] |= ((base >> 24 & 0xFFULL)  << 56);
}
