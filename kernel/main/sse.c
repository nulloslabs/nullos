...
size_t get_fpu_state_size(void) {
    // init_sse() runs before any task is created, so this is always set.
...
void init_fpu_area(void *area) {
    // A clean saved area is all zeros except that the x87 control word (cwd)
    // and MXCSR need sane defaults — mirroring what fninit + ldmxcsr produce.
    // We zero the buffer, set the live FPU to a known state, then save it.
